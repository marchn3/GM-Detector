...
#define WARNING_PIN 13  // LED and Buzzer connected to digital pin 13
#define COUNT_THRESHOLD 40  // Threshold for counts
...
void setup(){             //setup subprocedure
  ...
  pinMode(WARNING_PIN, OUTPUT);  // Set the buzzer pin as output
  digitalWrite(WARNING_PIN, LOW);  // TEnsure warning starts turned off
...
void loop(){                                 //main cycle
  ...
  if(currentMillis - previousMillis > LOG_PERIOD){
    ...
    Serial.print(cpm);
    counts = 0;
  if(cpm >= COUNT_THRESHOLD){
    digitalWrite(WARNING_PIN, HIGH);  // Turn on the warning
    delay(15000);  // Keep them on for 15 seconds
    digitalWrite(WARNING_PIN, LOW);  // Turn off the warning
    }
  }
